...
import { BaseObject, Days, DetailedSatellite, PayloadStatus, SpaceObjectType, Star } from 'ootk';
...
export class ColorSchemeManager {
  ...
  geo(obj: BaseObject): ColorInformation {
    ...
    if (keepTrackApi.getDotsManager().inViewData?.[sat.id] === 1 && this.objectTypeFlags.celestrakDefaultFov === true) {
      return {
        color: this.colorTheme.inFOV,
        pickable: Pickable.Yes,
      };
    ...
    keepTrackApi.register({
      ...
      cb: (): void => {
        ...
        if (cachedColorScheme) {
          ...
          this.currentColorScheme = possibleColorScheme || this.celestrakDefault;
  ...
  async setColorScheme(scheme: ((obj: BaseObject, params?: any) => ColorInformation) | null, isForceRecolor?: boolean) {
    try {
      ...
      this.currentColorScheme = possibleColorScheme ? possibleColorScheme : this.celestrakDefault;
      ...
      console.debug(error);
      settingsManager.setCurrentColorScheme(this.celestrakDefault);
      this.currentColorScheme = this.celestrakDefault;
      this.calculateColorBuffers(isForceRecolor);
    }
  }
    ...
    if (!isForceRecolor) {
      switch (this.currentColorScheme) {
        ...
        default:
          ...
          this.updateColorScheme(this.celestrakDefault);
          break;
      }
    }
  }
